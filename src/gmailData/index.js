 ...
 */
async function listAndMarkAsRead(auth) {
  ...
  if (!emails || emails.length === 0) {
    console.log('No unread emails from notifications@oddsjam.com found in the inbox.');
    return;
  }
  ...
  for (const email of emails) {
    try {
      ...
      if (betOnText) {
        console.log('Text after "Bet on" until "odds on":', betOnText);
      } else {
        console.log('Text after "Bet on" until "odds on" not found in the email body.');
      }
